...
## 1) Integration Tests (Controller + Security)

> Scope: `ConsentController#getConsentHistory`

1. **401 when unauthenticated**
   - **Given** no authentication
   - **When** GET `/history/{userId}`
   - **Then** response is `401 UNAUTHORIZED` with error payload.

2. **403 when accessing another user’s history**
   - **Given** authenticated principal UUID `A`, path `{userId}` is UUID `B != A`
   - **When** GET `/history/B`
   - **Then** response is `403 FORBIDDEN` and no data returned.

3. **500 when principal is not a UUID (per current code path)**
   - **Given** authenticated principal name is non‑UUID (e.g., `john`)
   - **When** GET `/history/{any-uuid}`
   - **Then** response is `500 INTERNAL_SERVER_ERROR` with message “Invalid user authentication”.

4. **400 when path userId is not a valid UUID**
   - **Given** authenticated principal matches any valid UUID
   - **When** GET `/history/not-a-uuid`
   - **Then** `400 BAD_REQUEST` with message “Invalid user ID format”.

5. **Happy path: 200 and well‑formed paginated payload**
   - **Given** authenticated principal equals `{userId}`
   - **When** GET `/history/{userId}?page=0&size=5`
   - **Then** `200 OK` and body is `PaginatedConsentHistoryResponse` with expected fields: `userId, entries[], page, size, total, totalPages, hasNext, hasPrevious`.

6. **Pagination defaults applied**
   - **When** GET `/history/{userId}` without query params
   - **Then** server uses `page=0`, `size=20` and returns correctly computed metadata.

7. **400 for invalid pagination params**
   - **page < 0** → `400 BAD_REQUEST`
   - **size = 0** → `400 BAD_REQUEST`
   - **size > 100** → `400 BAD_REQUEST`.

8. **Empty history returns empty list + zero totals**
   - **Given** no ledger rows for user
   - **When** GET `/history/{userId}`
   - **Then** `entries=[]`, `total=0`, `totalPages=0`, `hasNext=false`, `hasPrevious=false`.

9. **Deterministic ordering (event‑time desc, tie‑break by createdAt desc) across pages**
   - **Given** multiple rows with identical `consentTimestamp` and differing `createdAt`
   - **When** paging over results
   - **Then** order is stable: first by `consentTimestamp DESC`, then `createdAt DESC` across page boundaries.

10. **Entries include GRANTED and WITHDRAWN statuses**
    - **Given** mixed ledger statuses
    - **When** GET `/history/{userId}`
    - **Then** both appear in `entries[]` with correct `consentStatus` values.

11. **`coveredKids` is distinct and sorted**
    - **Given** coverage with duplicates/out‑of‑order kid IDs
    - **When** GET `/history/{userId}`
    - **Then** `coveredKids` are unique and sorted (string order).

12. **`withdrawnConsentId` is present only for withdrawals**
    - **Given** GRANTED and WITHDRAWN entries
    - **Then** GRANTED rows have `withdrawnConsentId = null`; WITHDRAWN rows have non‑null linked ID.

13. **Field mapping/serialization sanity**
    - **Then** enum fields (`consentType`, `consentStatus`, `lawfulBasis`, `source`) serialize as strings; timestamps are ISO serialized; nulls remain null (e.g., `parentVerificationId` when absent).

14. **Large page within limit works**
    - **When** `size=100`
    - **Then** returns up to 100 entries and correct metadata.
